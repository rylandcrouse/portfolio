...
const Sample = ({title, emoji, repoURL}: Props) => {
  ...
  return (
    <Link href={`${repoURL}`}>
        <div onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)} className="sm:aspect-[7/8] aspect-[10/9] max-h-[70vh] w-full min-w-1/3 lg:px-10 px-14 py-10 max-w-[700px]
        flex flex-col justify-evenly items-center space-y-5
        sm:rounded-xl sm:border-neutral-900/30 sm:border border-b border-neutral-900/40
